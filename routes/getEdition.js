...
router.get("/edition/:id", async (req, res) => {
    ...
    try {
        await session.readTransaction(tx => tx
            .run(`MATCH((edition:Edition)-[w:WRITTEN_BY]->(author:Author)) MATCH(edition:Edition)-[e:EDITED_BY]->(editor:Editor) WHERE id(edition) = ${idEdition} AND id(editor) = ${idEditor} RETURN edition.title, author.name, editor.name`)
            .subscribe({
                onNext: record => {
                    res.render("edition", {
            ...
            .catch(err => {
                console.log("Error related to getting the data from Neo4j: " + err);
            })
        );
    } catch (err) {
